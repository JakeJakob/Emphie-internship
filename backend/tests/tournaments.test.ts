...
describe("POST /tournaments | adding tournament without ACCESS_KEY", () => {
	it("should return 401 UNAUTHORIZED because of no ACCESS_KEY", async () => {
		await request(app).post("/tournaments").send({ name: "Test tournament" }).expect("Content-Type", /json/).expect(401);
	});
});
...
describe("POST /tournaments | adding tournament with ACCESS_KEY", () => {
	beforeAll(async () => {
		const res = await request(app).post("/tournaments").send({ name: "Test tournament" }).expect("Content-Type", /json/).set("Authorization", header).expect(201);
	...
	it("should return 201 CREATED when adding a judge to the created tournament", async () => {
		await request(app).post(`/tournaments/${tournamentId}/judges`).send({ name: "judge" }).expect("Content-Type", /json/).set("Authorization", header).expect(201);
	});
});
